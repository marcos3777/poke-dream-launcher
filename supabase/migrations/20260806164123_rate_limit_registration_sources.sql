...
create table public.community_registration_limits (
  source_hash text primary key,
  registrations smallint not null default 0,
  created_at timestamptz not null default now(),
  expires_at timestamptz not null,
  constraint community_registration_limits_hash_format
    check (source_hash ~ '^[0-9a-f]{64}$'),
  constraint community_registration_limits_count_range
    check (registrations between 0 and 10),
  constraint community_registration_limits_expiry
    check (expires_at > created_at)
);

alter table public.community_registration_limits enable row level security;
alter table public.community_registration_limits force row level security;
revoke all privileges on table public.community_registration_limits from public, anon, authenticated;
grant select, insert, update, delete on table public.community_registration_limits to service_role;

create index community_registration_limits_expires_at_idx
  on public.community_registration_limits (expires_at);

drop function public.replace_hunt_stats(uuid, text, bigint, integer, text, jsonb);
...
begin
  ...
  if not found then
    -- Serializa somente novos cadastros. IDs jÃ¡ conhecidos nÃ£o disputam este lock.
    ...
    if not found then
      delete from public.community_registration_limits
      where expires_at <= v_now;

      insert into public.community_registration_limits (
        source_hash,
        registrations,
        expires_at
      )
      values (
        p_source_hash,
        0,
        pg_catalog.date_trunc('day', v_now, 'UTC') + interval '1 day'
      )
      ...
      if v_source.registrations >= 1 then
        v_retry_after := greatest(
          1,
          ceil(extract(epoch from (v_source.expires_at - v_now)))::integer
        );
        return query
        select 'registration_limited'::text, 0, 0::bigint, v_retry_after;
        return;
      end if;
