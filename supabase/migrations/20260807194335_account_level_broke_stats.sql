-- Community schema v2: keep one row per anonymous account and species. Existing v1 rows remain
-- readable as "legacy" until that installation sends its first v2 snapshot.
...
alter table public.hunt_stats
  ...
  add column shiny_caught bigint not null default 0,
  add column broke_max bigint,
  add column broke_min bigint,
  drop constraint hunt_stats_pkey,
  add constraint hunt_stats_pkey primary key (client_id, account_id, species),
  add constraint hunt_stats_account_id_format
    check (account_id = 'legacy' or account_id ~ '^[0-9a-f]{64}$'),
  add constraint hunt_stats_shiny_caught_range
    check (shiny_caught between 0 and 1000000000000 and shiny_caught <= shinies and shiny_caught <= caught),
  add constraint hunt_stats_broke_state
    check (
      (broke_max is null and broke_min is null)
      or (
        shiny_caught > 0
        and broke_min between 1 and broke_max
        and broke_max <= shinies
      )
    );

alter table public.hunt_stats alter column account_id drop default;

comment on table public.hunt_stats is
  'Latest complete community snapshot, one row per installation-scoped anonymous account and canonical species.';
comment on column public.hunt_stats.account_id is
  'HMAC-derived account identifier scoped to one installation; raw character IDs and names are never stored.';
...
begin
  ...
  if p_schema_version = 1 then
    v_required_keys := array[
      'kills', 'caught', 'shinies', 'thrown_a', 'thrown_b', 'caught_a', 'caught_b', 'ms'
    ];
  else
    v_required_keys := array[
      'kills', 'caught', 'shinies', 'shiny_caught', 'broke_max', 'broke_min',
      'thrown_a', 'thrown_b', 'caught_a', 'caught_b', 'ms'
    ];
  ...
  for v_account_id, v_account_stats in
    select 'legacy'::text, p_stats
    where p_schema_version = 1
    union all
    select account.key, account.value
    from jsonb_each(p_stats) as account
    where p_schema_version = 2
  loop
    if p_schema_version = 2 and v_account_id !~ '^[0-9a-f]{64}$' then
    ...
    for v_species, v_entry in
      ...
      from jsonb_each(v_account_stats) as entry
    loop
      v_count := v_count + 1;
      ...
      if jsonb_typeof(v_entry -> 'kills') <> 'number'
         ...
         or jsonb_typeof(v_entry -> 'ms') <> 'number'
         or (p_schema_version = 2 and jsonb_typeof(v_entry -> 'shiny_caught') <> 'number')
         or (p_schema_version = 2 and (v_entry -> 'broke_max') <> 'null'::jsonb and jsonb_typeof(v_entry -> 'broke_max') <> 'number')
         or (p_schema_version = 2 and (v_entry -> 'broke_min') <> 'null'::jsonb and jsonb_typeof(v_entry -> 'broke_min') <> 'number') then
      ...
      v_shiny_caught := case when p_schema_version = 2 then (v_entry ->> 'shiny_caught')::numeric else 0 end;
      v_broke_max := case when p_schema_version = 2 then (v_entry ->> 'broke_max')::numeric else null end;
      v_broke_min := case when p_schema_version = 2 then (v_entry ->> 'broke_min')::numeric else null end;
      ...
      if v_caught > v_kills
         ...
         or ((v_broke_max is null) <> (v_broke_min is null))
         or (v_shiny_caught = 0 and (v_broke_max is not null or v_broke_min is not null))
         or (v_broke_max is not null and (v_broke_min > v_broke_max or v_broke_max > v_shinies)) then
  ...
  insert into public.hunt_stats (
    client_id, account_id, species, kills, caught, shinies, shiny_caught, broke_max, broke_min,
  ...
  select
    ...
    case when p_schema_version = 2 then (entry.value ->> 'shiny_caught')::bigint else 0 end,
    case when p_schema_version = 2 then (entry.value ->> 'broke_max')::bigint else null end,
    case when p_schema_version = 2 then (entry.value ->> 'broke_min')::bigint else null end,
    ...
    v_now
  from (
    select 'legacy'::text as account_id, p_stats as account_stats
    where p_schema_version = 1
    union all
    select source.key, source.value
    from jsonb_each(p_stats) as source
    where p_schema_version = 2
  ) as account
  cross join lateral jsonb_each(account.account_stats) as entry;
...
revoke all privileges on function public.replace_hunt_stats(uuid, text, bigint, integer, text, jsonb, text)
  from public, anon, authenticated;
grant execute on function public.replace_hunt_stats(uuid, text, bigint, integer, text, jsonb, text)
  to service_role;
